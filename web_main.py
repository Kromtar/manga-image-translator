...
async def run_async(request):
    ...
    TASK_DATA[task_id] = {
        'translator': selected_translator, 'tgt': target_language}
        ...
        if task_id in TASK_DATA:
            ...
            while True:
                ...
                if TASK_STATES[task_id] == 'finished':
                    ret = web.json_response(
                        {'task_id': task_id, 'status': 'successful'})
                    break
            # remove old tasks
    ...
    if rqjson['nonce'] == NONCE:
        ...
        if task_id in TASK_DATA:
            if 'manual' in TASK_DATA[task_id]:
                ...
                asyncio.gather(machine_trans_task(
                    task_id, rqjson['texts'], TASK_DATA[task_id]['translator'], TASK_DATA[task_id]['tgt']))
    return web.json_response({})
...
async def get_task_state_async(request):
    ...
    if task_id and task_id in TASK_STATES:
        try:
            ret = web.json_response(
                {'state': TASK_STATES[task_id], 'waiting': QUEUE.index(task_id) + 1})
        except:
            ret = web.json_response(
                {'state': TASK_STATES[task_id], 'waiting': 0})
...
async def submit_async(request):
    ...
    if 'translator' in data:
        selected_translator = data['translator'].lower()
        if selected_translator not in ['youdao', 'baidu', 'google', 'null']:
            selected_translator = 'youdao'
    if 'size' in data:
        size = data['size'].upper()
        if size not in ['S', 'M', 'L', 'X']:
...
@routes.post("/get-translate-result")
async def manual_translate_async(request):
    task_id = request.query.get('taskid')
    if task_id is not None:
        try:
            sentenceFileRute = "result/"+task_id+"/sentences.txt"
            with open(sentenceFileRute, 'r') as sentenceFile:
                sentences = "".join(sentenceFile.readlines()[1:])
            return web.json_response({'sentences': sentences})
        except:
            return web.json_response({'detail': 'file not found', 'status': 'failed'})
    return web.json_response({'detail': 'invalid request, provide taskid', 'status': 'failed'})
